...
def main(argv):
    ...
    Cardiom = np.concatenate((y_predU2[:, 2, np.newaxis], y_predU16[:, 2, np.newaxis], y_predU19[:, 2, np.newaxis], y_predU21[:, 2, np.newaxis], y_predU22[:, 2, np.newaxis], y_predU23[:, 2, np.newaxis]), axis=1)
